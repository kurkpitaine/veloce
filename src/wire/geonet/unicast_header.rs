...
impl<T: AsRef<[u8]>> Header<T> {
    ...
    #[inline]
    pub fn destination_position_vector(&self) -> Result<ShortPositionVector> {
        let data = self.buffer.as_ref();
        let spv_buf = SPVBuf::new_unchecked(&data[field::DE_PV]);
        ShortPositionVector::parse(&spv_buf)
    }
}
...
impl<T: AsRef<[u8]> + AsMut<[u8]>> Header<T> {
    ...
    pub fn set_destination_position_vector(&mut self, value: ShortPositionVector) {
        let data = self.buffer.as_mut();
        let mut spv_buf = SPVBuf::new_unchecked(&mut data[field::DE_PV]);
        value.emit(&mut spv_buf);
    }
}
...
mod test {
    ...
    static BYTES_HEADER: [u8; 48] = [
        ...
        0x78, 0x1c, 0xc6, 0x66, 0x60, 0xfd, 0xe2, 0x03, 0xd4, 0x80, 0x18, 0x0b, 0x2c, 0xbc, 0x00,
        0x9a, 0xf3, 0xd8, 0x02, 0xfb, 0xd1, 0x00, 0x00, 0x00, 0x78, 0x1c, 0xc6, 0x66, 0x60, 0xfd,
        0xe2, 0x03, 0xd4,
    ];
    ...
    fn spv_repr() -> ShortPositionVector {
        ShortPositionVector {
            address: GnAddress::new(
                true,
                StationType::RoadSideUnit,
    ...
    fn test_deconstruct() {
        ...
        assert_eq!(header.destination_position_vector().unwrap(), spv_repr());
    }
    ...
    fn test_repr_parse_valid() {
        ...
        assert_eq!(
            ...
            Repr {
                ...
                destination_position_vector: spv_repr(),
            }
        );
    }
