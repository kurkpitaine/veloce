...
/// Wait until one of the given files descriptors becomes readable, but no longer than given timeout.
///
/// # Panics
///
/// This method panics when `fds` is empty.
pub fn wait_many(fds: &Vec<RawFd>, duration: Option<Duration>) -> io::Result<()> {
    if fds.is_empty() {
        panic!("Vector of files descriptors empty!");
    }
    ...
    unsafe {
        ...
        let res = libc::select(
            fds.iter().max().unwrap() + 1, // Safety: we checked earlier the vector is not empty.
            &mut readfds,
            &mut writefds,
            &mut exceptfds,
            timeout_ptr,
